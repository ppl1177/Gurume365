...
function permitUser(){
	...
	$.ajax({
		...
		type: 'post',
		data: {
			permitToken : (document.getElementById('permitToken')).value
		},
		dataType: 'json',
		success: function(dat) {
			if(dat!=0) {
				alert('인증 완료\n 다시 로그인 해주시기 바랍니다.');
				location.href="/gurume365/logout";
			}
			else {
				alert('인증 실패');
			}
...
<body>
    ...
    <div class="mainInfo" align="right">
    	...
    	<div class="left">
    		...
    		<h2 style="color: white; position: relative; bottom: 15px;">본인 인증</h2>
    		</div>
...
<div>
<h3 class="text">본인 인증</h3>
<div class="passworddiv">
<h5>인증번호를 입력해주세요</h5>
<input type="text" id="permitToken" class="withdrawPw">
</div>
<input type="button" class="submitButton" value="인증" onclick="permitUser()">
<input type="button" class="withdrawcancel" onclick="cancel()" value="취소">
</div>


<h5 class="farewell">꼬레까라 요로시쿠 오네가이시마스~</h5>
</div>
</div>
</div>
</div>
